# Автоматизация системы контроля качества проб воды
...
"норма/превышение" по каждой пробе и дашбордом по месяцам.
...
было бы потратить на сами анализы, а не на их оформление.
...
- концентрация, статус по каждому параметру (норма/превышение
  ...
  формулам** — без единого ручного пересчёта;
- всё сразу попадает в общий свод с датой, номером пробы и статусом —
  не нужно вести отдельные журналы и переносить цифры между листами;
- добавлен дашборд: доля проб, прошедших контроль, по месяцам и
  разбивка превышений по параметрам — раньше такой картины по времени
  просто не существовало, каждый месяц смотрели "с нуля".

## Результат

**Время на пробу: 15 мин → 4 мин (-73%)**
...
├── excel/
│   └── qc_workbook.xlsx            # готовая Excel-система с формулами
├── results/
